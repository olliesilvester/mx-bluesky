...
lg.basicConfig(format='%(asctime)s %(levelname)s:   \t%(message)s',level=lg.DEBUG, filename=time.strftime("logs/i24_%d%B%y.log").lower())
#### Old logging
    ...
    if location == 'i24':
        ...
        for line in f:
            ...
            if 'det_type' in entry[0].lower():
                det_type = entry[1]
        return chip_name, visit, sub_dir, n_exposures, chip_type, map_type, pump_repeat, pumpexptime, pumpdelay, exptime, dcdetdist, prepumpexptime, det_type
...
def write_file(location='i24', suffix='.addr', order='alphanumeric'):
    ...
    lg.info('%s'%name)
    if location == 'i24':
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, pump_repeat, pumpexptime, exptime, dcdetdist, prepumpexptime = scrape_parameter_file('i24')
...
def check_files(location, suffix_list):
    name = inspect.stack()[0][3]
    if location == 'i24':
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, exptime, pump_repeat, pumpdelay, pumpexptime, dcdetdist, prepumpexptime, det_type = scrape_parameter_file('i24')
        a_directory = '/dls_sw/i24/scripts/fastchips/'
    elif location == 'SACLA':
        chip_name, sub_dir, n_exposures, chip_type, map_type = scrape_parameter_file()
        a_directory = '/localhome/local/Documents/sacla/'
...
def write_headers(location, suffix_list):
    name = inspect.stack()[0][3]
    if location == 'i24':
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, pump_repeat, pumpexptime, pumpdelay, exptime, dcdetdist, prepumpexptime, det_type = scrape_parameter_file('i24')
        a_directory = '/dls_sw/i24/scripts/fastchips/'
    elif location == 'SACLA':
        chip_name, sub_dir, n_exposures, chip_type, map_type = scrape_parameter_file()
        a_directory = '/localhome/local/Documents/sacla/'
