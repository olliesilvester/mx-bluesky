...
from ca_py3 import caput, caget, cagetstring
import logging as lg
import setup_beamline_py3 as sup
from i24ssx_Chip_StartUp_py3v1 import scrape_parameter_file, read_parameters
from i24ssx_Chip_StartUp_py3v1 import get_format
...
def get_chip_prog_values(chip_type, location, pump_repeat, pumpexptime, pumpdelay, prepumpexptime, exptime=16, n_exposures=1):
...
def datasetsizei24():
    ...
    name = inspect.stack()[0][3]
    chip_name, visit, sub_dir, n_exposures, chip_type, map_type, pump_repeat, pumpexptime, pumpdelay, exptime, dcdetdist, prepumpexptime, det_type = scrape_parameter_file(location='i24')
...
def start_i24():
    ...
    if det_type == 'pilatus':
        ...
        lg.info("%s Fastchip Pilatus setup: number of images %s" % (name, total_numb_imgs))
        ...
        lg.info("%s Triggered Eiger setup: number of images %s" % (name, total_numb_imgs))
...
def call_nexgen(chip_prog_dict, start_time):
    ...
    if det_type == 'eiger':
        print("nex gen here")
        currentchipmap = '/dls_sw/i24/scripts/fastchips/litemaps/currentchip.map' if map_type != 0 else "fullchip"
...
def main(location='i24'):
    ...
    if location == 'i24':
        chip_name, visit, sub_dir, n_exposures, chip_type, map_type, pump_repeat, \
                               pumpexptime, pumpdelay, exptime, dcdetdist, prepumpexptime, det_type = scrape_parameter_file(location='i24')
        ...
        chip_name, sub_dir, n_exposures, chip_type, map_type, prepumpexptime = scrape_parameter_file(location='SACLA')
    ...
    if location =='i24':
        chip_prog_dict = get_chip_prog_values(chip_type, location, pump_repeat, pumpexptime, pumpdelay, prepumpexptime, exptime=exptime, n_exposures=n_exposures)
    else:
