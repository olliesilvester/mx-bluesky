...
def get_beam_centre(oav: OAV, oav_config: Dict[str, str] = OAV_CONFIG_FILES):
    ...
    oav_params.zoom = read_zoom_level(oav)
...
def onMouse(event, x, y, flags, param):
    beamX, beamY = param
    if event == cv.EVENT_LBUTTONUP:  # cv.EVENT_LBUTTONUP is an int!
...
if __name__ == "__main__":
    ...
    beamX, beamY = get_beam_centre(oav)
    ...
    cv.setMouseCallback("OAV1view", onMouse, param=(beamX, beamY))  # type: ignore
